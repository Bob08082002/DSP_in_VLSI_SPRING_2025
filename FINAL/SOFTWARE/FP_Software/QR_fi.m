...
function [x_outs,y_outs] = CORDIC(stages, cordicIWL, cordicFWL, x_ins, y_ins)
    ...
    for i = 0:N-1
        ...
        x_ins = fi(x_ins,true,cordicFWL+cordicIWL,cordicFWL);
        y_ins = fi(y_ins,true,cordicFWL+cordicIWL,cordicFWL);
        % 
        % fprintf("stage %d cordic\n", i);
        % disp(bin(x_ins))
        % disp(bin(y_ins))
    end
    ...
    x_outs_tmp = fi(x_outs,true,cordicFWL+cordicIWL,cordicFWL);
    y_outs_tmp = fi(y_outs,true,cordicFWL+cordicIWL,cordicFWL);
...
function result = csd_multiply(x_fi, csd_str, IWL, FWL)
    ...
    for i = 1:word_len
        ...
        disp(csd_str(i))
        if csd_str(i) == '1'
            fprintf("%s right shift %d. add to result\n", bin(x_fi), right_shift_amt);
            result = result + bitshift(x_fi, -right_shift_amt);
        elseif csd_str(i) == 'n'
            fprintf("%s right shift %d. sub from result\n", bin(x_fi), right_shift_amt);
            result = result - bitshift(x_fi, -right_shift_amt);
        end
    end
