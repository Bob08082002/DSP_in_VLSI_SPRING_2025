function [R_fi,z_fi,x_sol_fi,suggested_cordicIWL,suggested_backsubIWL] = QR_fi( stages, cordicIWL,cordicFWL, ...
    ...
    % x4 = z4 / r44;
    r44_recip = divide(T,fi(1,true,total_bits,frac_bits), R_backsub(4,4));
    x_sol_fi(4) = z_backsub(4) * r44_recip;%divide(T,z_backsub(4),R_backsub(4,4));
    
    % x3 = (z3 - x4 * r34) / r33;
    tmp_ = z_backsub(3) - x_sol_fi(4) * R_backsub(3,4);
    r33_recip = divide(T,fi(1,true,total_bits,frac_bits), R_backsub(3,3));
    x_sol_fi(3) = r33_recip * tmp_;%divide(T,tmp_,R_backsub(3,3));
    all_backsub_results = [all_backsub_results;double(x_sol_fi(4) * R_backsub(3,4)); double(tmp_); r33_recip ;r44_recip];

    % x2 = (z2 - x3*r23 - x4*r24) / r22;
    tmp_ = (z_backsub(2) - x_sol_fi(3) * R_backsub(2,3) - x_sol_fi(4) * R_backsub(2,4));
    r22_recip = divide(T,fi(1,true,total_bits,frac_bits), R_backsub(2,2));
    x_sol_fi(2) = tmp_ * r22_recip; %divide(T,tmp_,R_backsub(2,2));
    all_backsub_results = [all_backsub_results; double(x_sol_fi(4) * R_backsub(2,4)); double(x_sol_fi(3) * R_backsub(2,3)); 
                           double( z_backsub(2) - x_sol_fi(3) * R_backsub(2,3)); double(tmp_); r22_recip];
    ...
    r11_recip = divide(T,fi(1,true,total_bits,frac_bits), R_backsub(1,1));
    x_sol_fi(1) = r11_recip * tmp_; %divide(T,tmp_,R_backsub(1,1));
    all_backsub_results = [all_backsub_results; double(x_sol_fi(2) * R_backsub(1,2)); double(x_sol_fi(3) * R_backsub(1,3)); double(x_sol_fi(4) * R_backsub(1,4)); 
                            ...
                            double(tmp_); double(x_sol_fi(:)); r11_recip];

    % fprintf("R_backsub(4,4): %s\n", bin(R_backsub(4,4)));
    % fprintf("r44_recip: %s\n", bin(r44_recip));
    % fprintf("R_backsub(3,3): %s\n", bin(R_backsub(3,3)));
    % fprintf("r33_recip: %s\n", bin(r33_recip));
    % fprintf("R_backsub(2,2): %s\n", bin(R_backsub(2,2)));
    % fprintf("r22_recip: %s\n", bin(r22_recip));
    % fprintf("R_backsub(1,1): %s\n", bin(R_backsub(1,1)));
    % fprintf("r11_recip: %s\n", bin(r11_recip));

    %% Analyze the maximum value of all_cordic_results and all_backsub_results
    max_val = max(abs(all_cordic_results));
    suggested_cordicIWL = ceil(log2(max(double(max_val),1)))+1;

    max_val = max(abs(all_backsub_results));
    suggested_backsubIWL = ceil(log2(max(double(max_val),1)))+1;
