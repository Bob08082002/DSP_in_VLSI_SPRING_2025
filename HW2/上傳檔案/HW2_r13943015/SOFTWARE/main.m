...
elseif mode == "bitwidth_vs_error_datatype_combined"
    ...
    plot(w/pi, 20*log10(abs(H_fix))); % Convert magnitude to dB
    xlabel('Normalized Frequency (\times\pi rad/sample)');
    ylabel('Magnitude (dB)');
    title('Magnitude Response of SRRC FIR Filter (fixed point)');
    grid on;
    subplot(2,1,2); % Plot phase response in radians
    plot(w/pi, angle(H_fix)); % Phase response in radians
    xlabel('Normalized Frequency (\times\pi rad/sample)');
    ylabel('Phase (radians)');
...
elseif mode == "datatype_combined_save_coeff_and_input"
    % save fixed point coeff
    fraction_bitwidth = quantize_bitwidth_setting(2, 2);
    total_bitwidth = quantize_bitwidth_setting(2, 1) + quantize_bitwidth_setting(2, 2);
    h_fixed = fi(h, 1, total_bitwidth, fraction_bitwidth, 'RoundingMethod', 'Nearest');
    
    fileID = fopen(coeff_file_path, 'w');
    for i = 1:length(h_fixed)
        bin_data = bin(h_fixed(i));
        fprintf(fileID, '%s\n', bin_data);  % 每個數值換行
    end
    fclose(fileID);


    % save fixed point input
    fraction_bitwidth = quantize_bitwidth_setting(1, 2);
    total_bitwidth = quantize_bitwidth_setting(1, 1) + quantize_bitwidth_setting(1, 2);
    x_fixed = fi(x, 1, total_bitwidth, fraction_bitwidth, 'RoundingMethod', 'Nearest');
    
    fileID = fopen(input_file_path, 'w');
    for i = 1:length(x_fixed)
        bin_data = bin(x_fixed(i));
        fprintf(fileID, '%s\n', bin_data);  % 每個數值換行
    end
    fclose(fileID);

elseif mode == "hardware_report_p5"
    % Read the file
    bin_strings = readlines(output_file_path);
    bin_strings = bin_strings(strlength(bin_strings) > 0);  % remove last line(\n)
    % Convert binary strings to fixed-point numbers using fi (Fixed-Point Designer toolbox)
    fraction_bitwidth = quantize_bitwidth_setting(4, 2);
    total_bitwidth = quantize_bitwidth_setting(4, 1) + quantize_bitwidth_setting(4, 2);
    % Convert binary strings directly using fi
    hardware_y_fixed = fi(0, true, total_bitwidth, fraction_bitwidth); % Define fixed-point type
    hardware_y_fixed.bin = bin_strings; % Assign binary values

    
    % Compute error
    errors = abs(double(hardware_y_fixed) - y_float(:));
    ...
    plot(errors,  'k');
    xlabel('n');
    ylabel('Error |hardware\_y\_fix - y\_float|');
    title('Error between hardware y\_fix and y\_float');
    grid on;
else
