...
function result = TF32_add(operand_A , operand_B)
    ...
    sign_bit_2 = sign_mantissa_B_prec(1);
    % 執行右移：去掉最後 n 位，前面補 n 個符號位
    shifted_str_2 = [repmat(sign_bit_2, 1, exp_diff), sign_mantissa_B_prec(1:end-exp_diff)];
   
    if Is_A_Exp_L
        Shifted_sign_mantissa_A_prec = sign_mantissa_A_prec;
        Shifted_sign_mantissa_B_prec = string_to_fixpoint(bin(shifted_str_2) , 12+PRECISION_BIT_LEN, 0);
        
    else
        Shifted_sign_mantissa_A_prec = string_to_fixpoint(bin(shifted_str_1) , 12+PRECISION_BIT_LEN, 0);
